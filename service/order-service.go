...
import (
	...
	"github.com/elivoa/syd/dal/orderdao"
...
func (s *OrderService) GetOrders(params model.Params, withs Withs) (
	orders []*model.Order, total int, err error) {

	// fmt.Println(">>>>>>>>>>>>> lllll ", params)
	...
	// 2. get order list.
	parser.Limit(params.Int("current") * params.Int("page_items"), params.Int("page_items")) // pager
	orders, err = s.ListOrders(parser, withs)
	return
}

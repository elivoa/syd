...
type Order struct {
	...
	TrackNumber int64  `json:"tn,omitempty"`          // real identification, tracking number
	Status      string `json:"status,omitempty"`      // todeliver | delivering | done | canceled | (all)
	Type        uint   `json:"type,omitempty"`        // OrderType  // 代发 | 大货 | 子订单
	CustomerId  int    `json:"customer_id,omitempty"` // TODO: change this into int64

	// shipping info
	DeliveryMethod         string `json:"delivery_method,omitempty"`  // YTO, SF, Depoon, Freight, TakeAway
	DeliveryTrackingNumber string `json:"delivery_tn,omitempty"`      // 快递单号
	ExpressFee             int64  `json:"express_fee,omitempty"`      // -1 means 到付
	ShippingAddress        string `json:"shipping_address,omitempty"` // this only used in ShippingInstead

	// price summarization.
	TotalPrice  float64 `json:"total_price,omitempty"` // not include expressfee
	TotalCount  int     `json:"total_count,omitempty"`
	PriceCut    float64 `json:"price_cut,omitempty"`   // currently not used.
	Accumulated float64 `json:"accumulated,omitempty"` // 上期累计欠款快照（不包含本期订单价格以及代发费用）

	Note              string `json:"note,omitempty"`
	ParentTrackNumber int64  `json:"parent_tn,omitempty"` // if has value it's a suborder

	Details []*OrderDetail `inject:"slice" json:"details,omitempty"` // cascated
